 ...
 */
class pdf_azur_mentionva extends ModelePDFPropales
{
	/**
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->setXY($this->marge_gauche,$posy+36);
        $pdf->SetFont('','', $default_font_size-3);
		$pdf->setTextColor(200,0,0);
        $yBefore = $pdf->getY();
		$pdf->MultiCell(0,2,$outputlangs->convToOutputCharset($object->array_options ['options_mention']), 1, "L");
        $posy = $pdf->getY() + 3;
		$top_shift = $top_shift + ($posy - $yBefore);
        $pdf->SetTextColor(0,0,0);
