...
$hookmanager->initHooks(array('showlist'));
...
llxHeader('', $langs->trans('showList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->show->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'show t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'show_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('show', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_show', 'GET');

$nbLine = !empty($user->conf->MAIN_SIZE_LISTE_LIMIT) ? $user->conf->MAIN_SIZE_LISTE_LIMIT : $conf->global->MAIN_SIZE_LISTE_LIMIT;

$r = new Listview($db, 'show');
echo $r->render($sql, array(
	...
	,'search' => array(
		...
		,'status' => array('search_type' => show::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
